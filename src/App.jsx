...
function scoreColor(s) { if (s >= 80) return SAFE; if (s >= 40) return WARN; return DANGER; }
function verdict(s) { if (s >= 80) return "HIGH TRUST"; if (s >= 60) return "MODERATE TRUST"; if (s >= 40) return "ELEVATED RISK"; return "HIGH RISK"; }
function calcScore(ratings) {
  let total = 0;
  for (const dim of DIMENSIONS) { if (ratings[dim.key]) total += (ratings[dim.key] / 5) * dim.weight * 100; }
  return Math.round(total);
}
function getRatingColor(val) { if (val <= 1) return DANGER; if (val <= 3) return WARN; return SAFE; }
function getRatingBg(val) { if (val <= 1) return "rgba(224,82,82,0.18)"; if (val <= 3) return "rgba(224,168,50,0.18)"; return "rgba(50,200,122,0.15)"; }
...
export default function TRMApp() {
  ...
  async function loadCounterparties(userId) {
    const { data } = await supabase.from("counterparties").select("*").eq("user_id", userId).order("updated_at", { ascending: false });
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      const { data: cpData } = await supabase.from("counterparties").upsert({
        user_id: session.user.id, name: form.name, country: form.country,
        tx_type: form.txType, tx_value: form.txValue, rel_source: form.relSource,
        ratings, score, updated_at: new Date().toISOString()
      }).select().single();

      if (cpData) {
        await supabase.from("assessments").insert({
          counterparty_id: cpData.id, user_id: session.user.id,
          score, ratings, ...parsed
        });
        setActiveCP(cpData);
      }
      ...
      setStep("results");
      loadCounterparties(session.user.id);
    } catch (e) {
      clearInterval(msgInterval);
  ...
  function reset() { setStep("form"); setRatings({}); setForm({ name: "", country: "", txType: "", txValue: "", relSource: "", notes: "" }); setResult(null); setError(""); setActiveCP(null); }

  function openCP(cp) {
    setActiveCP(cp);
    setForm({ name: cp.name, country: cp.country || "", txType: cp.tx_type || "", txValue: cp.tx_value || "", relSource: cp.rel_source || "", notes: "" });
    if (cp.ratings) setRatings(cp.ratings);
    if (cp.score) { setResult({ score: cp.score, name: cp.name, country: cp.country, txType: cp.tx_type, txValue: cp.tx_value, ratings: cp.ratings || {}, timestamp: cp.updated_at }); setStep("results"); }
    else { setStep("form"); setResult(null); }
    setView("assessment");
  }
  ...
  const s = {
    ...
    inner: { position: "relative", zIndex: 2, maxWidth: 920, margin: "0 auto", padding: "0 20px 80px" },
    ...
    userEmail: { fontFamily: "'Share Tech Mono', monospace", fontSize: 9, color: SILVER_DIM, letterSpacing: "0.1em" },
    headerBtn: (col) => ({ padding: "7px 16px", background: "transparent", border: "1px solid " + (col || "rgba(16,157,206,0.3)"), color: col || CYAN, fontFamily: "'Share Tech Mono', monospace", fontSize: 9, letterSpacing: "0.15em", cursor: "pointer" }),
    ...
    sectionTitle: { fontFamily: "'Rajdhani', sans-serif", fontSize: 28, fontWeight: 700, color: "#F0F4F6", letterSpacing: "0.04em", lineHeight: 1.1, marginBottom: 8 },
    sectionDesc: { fontSize: 13, color: SILVER_DIM, fontWeight: 300, lineHeight: 1.6, maxWidth: 560, marginBottom: 28 },
    ...
    cpCard: { background: BG2, border: "1px solid rgba(16,157,206,0.18)", padding: 18, marginBottom: 10, cursor: "pointer", transition: "border-color 0.2s" },
    cpName: { fontFamily: "'Rajdhani', sans-serif", fontSize: 17, fontWeight: 600, color: "#F0F4F6", marginBottom: 4 },
    cpMeta: { fontFamily: "'Share Tech Mono', monospace", fontSize: 9, color: SILVER_DIM, letterSpacing: "0.1em" },
    newBtn: { width: "100%", padding: 14, background: "transparent", border: "1px solid " + CYAN, color: CYAN, fontFamily: "'Rajdhani', sans-serif", fontSize: 14, fontWeight: 600, letterSpacing: "0.2em", textTransform: "uppercase", cursor: "pointer", marginBottom: 24 },
  };
      ...
      <div style={s.inner}>
        ...
        {view === "list" && (
          <div style={{ animation: "fadeUp 0.4s ease" }}>
            <div style={s.sectionLabel}>// COUNTERPARTY INTELLIGENCE</div>
            <div style={s.sectionTitle}>Your Counterparties</div>
            <div style={s.sectionDesc}>Manage and assess all your counterparties. Each profile saves assessment scores and correspondence history.</div>
            <button style={s.newBtn} onClick={() => { reset(); setView("assessment"); }}>+ NEW COUNTERPARTY ASSESSMENT</button>
            {counterparties.length === 0 && <div style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: 10, color: SILVER_DIM, letterSpacing: "0.1em", textAlign: "center", padding: "40px 0" }}>NO COUNTERPARTIES YET. START YOUR FIRST ASSESSMENT.</div>}
            {counterparties.map(cp => (
              <div key={cp.id} style={s.cpCard} onClick={() => openCP(cp)}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <div style={s.cpName}>{cp.name}</div>
                    <div style={s.cpMeta}>{[cp.country, cp.tx_type].filter(Boolean).join(" // ")}</div>
                  </div>
                  {cp.score && <div style={{ fontFamily: "'Rajdhani', sans-serif", fontSize: 28, fontWeight: 700, color: scoreColor(cp.score) }}>{cp.score}<span style={{ fontSize: 12, color: SILVER_DIM }}>/100</span></div>}
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ animation: "fadeUp 0.4s ease" }}>
            {step === "form" && (
              <>
                ...
                <div style={s.sectionDesc}>Rate all six trust dimensions to generate a scored AI risk report.</div>
                ...
                <div style={{ marginBottom: 28 }}><label style={s.formLabel}>Relationship Source</label><select style={s.select} value={form.relSource} onChange={e => setField("relSource", e.target.value)}>{REL_SOURCES.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}</select></div>
                <hr style={s.divider}/>
              ...
              <>
                ...
                <div style={{ marginBottom: 28 }}>
                  {DIMENSIONS.map(dim => {
                    ...
                    return (
                      <div key={dim.key} style={{ display: "grid", gridTemplateColumns: "170px 1fr 36px", alignItems: "center", gap: 12, marginBottom: 10 }}>
                ...
                <div style={s.analysisBlock}>
                  ...
                  {result.flags && result.flags.map((flag, i) => (
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 20, borderTop: "1px solid rgba(16,157,206,0.2)", marginTop: 20 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button style={s.resetBtn} onClick={() => { setView("list"); reset(); }}>ALL COUNTERPARTIES</button>
                  </div>
                </div>
              </>
            )}
